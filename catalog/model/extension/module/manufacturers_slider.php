<?php
class ModelExtensionModuleManufacturersSlider extends Model {
    ...
    public function getAllManufacturersByOrder($id_list) {
        $str_list = implode(',',$id_list);
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "manufacturer m WHERE m.manufacturer_id IN ({$str_list}) ORDER BY m.manufacturer_id DESC");
